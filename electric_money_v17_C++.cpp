...
#include <functional>
#include <cctype>
// Electric Money V17 Post-Quantum — C++17 consensus/reference port
// Derived from electric_money_v17_Post-Quantum.py.
// NOT production/mainnet cryptocurrency software.
// Requires OpenSSL 3.5+ (ML-DSA-65) and json-c.
// Build: g++ -std=c++17 -O2 -pthread electric_money_v17_post_quantum_1to1.cpp -lcrypto -ljson-c -o electric_money
...
static constexpr int DIFFICULTY_INTERVAL=10;
...
static constexpr int PROTOCOL_VERSION=16;
...
static const std::string NETWORK_ID="ELECTRIC-MONEY-TESTNET-V16-PQ";
...
class Blockchain {
    ...
    static void credit(std::map<std::string,i64>&b,const std::string&a,i64 x) {
        if(x<0 || b[a]>MAX_SUPPLY-x)throw std::runtime_error("balance overflow");
        b[a]+=x;
    }
    ...
    static void annual(std::map<std::string,i64>&b,std::map<std::string,i64>&a,i64&treas,i64&burn,i64 ts) {
        for(auto&[addr,anchor0]:a) {
            i64 anchor=anchor0;
            if(ts<anchor+YEAR_SECONDS)continue;
            int cycles=(int)std::max<i64>(1,(ts-anchor)/YEAR_SECONDS);
    ...
    static bool apply_tx(const Transaction&t,std::map<std::string,i64>&b,std::map<std::string,i64>&n,i64&burn,i64&treas) {
        ...
        if(n[s]!=t.nonce||b[s]<t.amount)return false;
        debit(b,s,t.amount);
        credit(b,t.recipient,t.net_amount());
        if(t.nonce==std::numeric_limits<i64>::max())return false;
        n[s]=t.nonce+1;
        burn+=t.burned();
        treas+=t.receiver_tax();
        return true;
    }
    bool validate_block(const Block&b,const Block&p,const std::vector<Block>&prefix,std::map<std::string,i64>bal,std::map<std::string,i64>nc,i64 issued,i64 burned,i64 treas,std::map<std::string,cpp_int>mw,std::map<std::string,i64>anchors,std::map<std::string,i64>*outbal=nullptr,std::map<std::string,i64>*outnc=nullptr,i64*outissued=nullptr,i64*outburn=nullptr,i64*outtreas=nullptr,std::map<std::string,cpp_int>*outmw=nullptr,std::map<std::string,i64>*outanchors=nullptr,i64 now=now_s())const {
        ...
        for(size_t i=0;i+1<b.transactions.size();++i) {
            ...
            if(typ=="transfer") {
                ...
                for(const char*k: {
                    ...
                    m[k]=json_escape(json_object_get_string(rr));
                    if(std::string(k)=="count"||std::string(k)=="first_sequence"||std::string(k)=="last_sequence")m[k]=json_object_to_json_string(rr);
                }
                if(id!=hash_obj( {
                     {
                        "count",m["count"]
        ...
        if(issued>MAX_SUPPLY||burned>issued)return false;
